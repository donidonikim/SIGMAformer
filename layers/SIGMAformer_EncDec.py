...
class EncoderLayer(nn.Module):
    ...
    def forward(self, x, attn_mask=None):
        x = x + self.multi_correlation(x, x)
        x, _ = self.decomp1(x)
        y = x
...
class DecoderLayer(nn.Module):
    def __init__(self, multi_correlation, cross_multi_correlation, d_model, c_out, d_ff=None, moving_avg=25, dropout=0.1, activation="relu"):
    ...
    def forward(self, x, cross, x_mask=None, cross_mask=None):
        ...
        x = x + self.cross_multi_correlation(x, cross)
